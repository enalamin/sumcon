    ...
    <!-- Content Header (Page header) -->
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Accounts Sub Ledger</li>
          </ol>
        </section>
        ...
        <section >
            <div class="box-body">
                <form role="form" action="<?php echo base_url()?>accounts/accountssubledger" method="post">
                  ...
                  <div class="col-md-2">
                    ...
                    <select class="form-control select2" name="accountHeadId" id="accountHeadId" >
                      <option value="">Select Accounts Head</option>
                      <?php
                          if(is_array($chartOfAccountsList)){
                              foreach ($chartOfAccountsList as $accountsHead){
                                if(in_array($accountsHead["accounts_head_id"],array(3,5,12))){
                      ?>
        ...
        <section class="invoice">
          ...
          <div class="row no-print">
            <div class="col-xs-12">
                <a href="<?php echo base_url(); ?>accounts/printaccountssubledger<?php echo "/".$accountHeadDetails[0]['accounts_head_id'];?><?php echo "/".$clientDetails[0]['client_id'];?><?php echo isset($fromDate)?"/".date('Y-m-d', strtotime($fromDate)):"";?><?php echo isset($toDate)?"/".date('Y-m-d', strtotime($toDate)):"";?>" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Print</a>
            </div>
          </div>
          <!-- info row -->
            ...
            <div class="col-xs-12" style="text-align: center;">
                    ...
                    <?php
                      if($accountHeadDetails){
                        ?>
                <div><h3>Accounts Head Name:  <?php echo $accountHeadDetails[0]['head_name']; ?><h3></div>
                  <?php
                }
                  ?>
              ...
              <table class="table table-striped table-bordered">
                ...
                <tbody>
                    <tr>
                        ...
                        <td align="right"></td>
                        <td align="right"><?php echo number_format(($openingBalance+$balanceAcStart),2);?></td>
                    </tr>
                    <?php
                        $i=0;
        						$totalDrAmount = $totalCrAmount = 0;
        						$currentBalance = $openingBalance+$balanceAcStart;
        						foreach ($transectionData as $transection){
                      $totalDrAmount += $transection['drAmount'];
                      $totalCrAmount += $transection['crAmount'];

        							
                      $currentBalance += $clientDetails[0]['client_type']=='Debtor'?$transection['drAmount']-$transection['crAmount']:$transection['crAmount']-$transection['drAmount'];
                  ?>
                  <tr>
