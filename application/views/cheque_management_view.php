...
<div class="wrapper">
	...
	<div class="content-wrapper">
		...
		<section class="content-header">
			<h1>
				Cheque Management
				<small>List of Collected Cheque</small>
			</h1>
			<ol class="breadcrumb">
				<li><a href="<?php echo base_url()?>"><i class="fa fa-dashboard"></i> Home</a></li>
				<li class="active">Cheque Management</li>
				<li class="active">List of Collected Cheque</li>
			</ol>
		</section>
							...
							<table id="example1" class="table table-bordered table-striped">
								<thead>
									<tr>
										...
										<th>Bank Name</th>
										<th>Cheque No</th>
                    <th align="right">Collection Amount</th>
										<th>Cheque Date</th>
										<th></th>
									</tr>
								</thead>
                    ...
                    <?php
                        if(is_array($collections) && !empty($collections)){
                            foreach ($collections as $collection){
                                if($collection['deposit_status']=='clear' || $collection['deposit_status'] == 'bounce')
                                    continue;
                    ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    ?>
                                            <a href="javascript:void(0);" class="btn btn-block btn-primary" onclick="clearCheque(<?php echo $collection["deposit_id"];?>,<?php echo $collection["collection_id"];?>,'<?php echo $collection["collection_no"];?>')">Clear</a>
                                            <a href="javascript:void(0);" class="btn btn-block btn-primary" onclick="bounceCheque(<?php echo $collection["deposit_id"];?>,<?php echo $collection["collection_id"];?>,'<?php echo $collection["collection_no"];?>')">Bounce</a>
                                    <?php
                                        }
                                    ?>
                                </td>
...
<!-- page script -->
    <script>
      $(function () {
            $("#example1").DataTable({"order": [[ 7, "asc" ]]});
            $(".select2").select2();
            $("#datemask").datepicker();
      });
       function clearCheque(depositId,collectionId,chequeNumber)
       {
            ...
            if(chequeno==chequeNumber){
               if(collectionId>0){
                   $.ajax({
                       type: "POST",
                       url: '<?php echo base_url()?>chequemanagement/clearcheque/'+depositId+"?collection_id="+collectionId,
                       dataType: 'json',
      ...
      function bounceCheque(depositId,collectionId,chequeNumber)
      {
          ...
          if(chequeno==chequeNumber){
              if(collectionId>0){
                  $.ajax({
                      type: "POST",
                      url: '<?php echo base_url()?>chequemanagement/bouncecheque/'+depositId+"?collection_id="+collectionId,
                      dataType: 'json',
                      complete: function(data){
                          var responses = data.responseText;
                          if(responses!=0)
