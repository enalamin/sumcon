...
<head>
	...
	<title>Sumcon Biotechnology | Stock Transfer</title>
...
<div class="wrapper">
	...
	<div class="content-wrapper">
		<!-- Content Header (Page header) -->
			...
			<h1>
				Inventory
                                <small>Stock Transfer entry</small>
				
			</h1>
			<ol class="breadcrumb">
				<li><a href="<?php echo base_url()?>"><i class="fa fa-dashboard"></i> Home</a></li>
				<li class="active">Inventory </li>
                                <li class="active">Stock Transfer Entry</li>
			</ol>
		</section>
                ...
                <section class="content">
                    ...
                    <div class="box-header with-border">
                      <h3 class="box-title">Stock Transfer Entry</h3>
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <form role="form"  action="<?php echo base_url()?>stocktransfer/transfer" method="post">
                        <div class="form-group">
                            <div class="col-xs-3">
                                <label>Transfer Number</label>
                                <input type="text" class="form-control" placeholder="Enter ..." name="transferNumber" value="<?php echo $transferNumber; ?>">
                            </div>
                            <div class="col-xs-1">
                            </div>
                            <div class="col-xs-3">
                        ...
                        <div class="form-group">
                            <div class="col-xs-6">
                                ...
                                <select class="form-control select2" name="fromLocation" id="fromLocation" required>
                                    <option value="">Select Location</option>
                                    <?php
                                        if(is_array($locationList)){
                                            foreach ($locationList as $location){
                                                if($this->session->userdata('locationId')==$location["location_id"]){
                                    ?>
                                            <option value="<?php echo $location["location_id"];?>" <?php echo $this->session->userdata('locationId')==$location["location_id"]?"selected":""; ?>><?php echo $location["location_name"];?></option>
                                    <?php
                                            }
                                            }
                                        }
                            ...
                            <div class="col-xs-6">
                                ...
                                <select class="form-control select2" name="toLocation" id="toLocation" required>
                                    <option value="">Select Location</option>
                                    <?php
                                    echo $this->session->userdata('locationId');
                                        if(is_array($locationList)){
                                            foreach ($locationList as $location){
                                                if($this->session->userdata('locationId')!=$location["location_id"]){
                                    ?>
       ...
       $("#datemask").datepicker();
        ...
        $("#btn_add").click(function(){
            ...
            if(product!='' && productQuantity!='' && productQuantity>0){
                ...
                +"<td><input type='hidden' name='productid[]' value='"+product+"'><input type='hidden' name='productQty[]' value='"+productQuantity+"'><input type='hidden' name='productPackge[]' value='"+productPackage+"'><input type='hidden' name='productRemarks[]' value='"+productRmarks+"'><button type='button' class='removebutton' title='Remove this row'>X</button></td></tr>");
                restproductrow();
                i++;
                $("#totalItemRow").val(i);
            }
