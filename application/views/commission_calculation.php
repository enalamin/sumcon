...
<html>
    <head>
        ...
        <title>Sumcon Biotechnology | Commission Calculation</title>
           ...
           <!-- Content Header (Page header) -->
            <section class="content-header">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="active">Commission Calculation</li>
                </ol>
            </section>
            <!-- Main content -->
            <section class="invoice">
                <form role="form"  action="<?php echo base_url().'sales/calcualtesalescommission/'.$invoiceDetails[0]['invoice_id']; ?>" method="post">
                    <div class="row invoice-info">
                        <div class="col-sm-4 invoice-col">
                            ...
                            <address>
                                ...
                                <input type="hidden" name="clientId" value="<?php echo $invoiceDetails[0]['client_id'];?>" >
                            </address>
                        </div><!-- /.col -->
                        <div class="col-sm-4 invoice-col">
                        ...
                        <div class="col-sm-4 invoice-col">                        
                            ...
                            <input type="text" class="form-control" placeholder="Enter ..." id="commissionDate" value="<?php echo date('m/d/Y');?>" name="commissionDate">
                        </div><!-- /.col -->
                    </div><!-- /.row -->
    				<div class="row">
                        <div class="col-xs-12 table-responsive">
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td><?php echo $invoice["quantity"].' '.$invoice["product_unit"];?><input type="hidden" name="invoiceQty[]" id="invoiceQty_<?php echo $invoice["product_id"];?>" value="<?php echo $invoice["quantity"];?>" ></td>
                                        <td><?php echo $invoice["unit_price"];?><input type="hidden" name="productsUnitPrice[]" id="productsUnitPrice_<?php echo $invoice["product_id"];?>" value="<?php echo $invoice["unit_price"];?>" ></td>
                                        <td><input type="text" class="form-control"  name="productAmount[]" id="productAmount_<?php echo $invoice["product_id"];?>" value="<?php echo ($invoice["delivered_qty"]*$invoice["unit_price"]);?>" readonly></td>
                                        <td >
                                            <select id="employee_<?php echo $invoice["product_id"];?>" name="employee[]"  class="form-control select2" style="width: 100%;">
                                        ...
                                        <td><input type="text" class="form-control"  name="commissionInKg[]" id="commissionInKg_<?php echo $invoice["product_id"];?>" value="" onblur="checkvalidqty(<?php echo $invoice["product_id"];?>)"></td>
                                        <td><input type="text" class="form-control"  name="commissionRate[]" id="commissionRate_<?php echo $invoice["product_id"];?>" value="" onblur="checkvalidqty(<?php echo $invoice["product_id"];?>)"></td>
                                        <td><input type="text" class="form-control"  name="commissionAmount[]" id="commissionAmount_<?php echo $invoice["product_id"];?>" value="" ></td>
                                        <td><input id="btn_add_<?php echo $invoice["product_id"];?>" name="btn_add[]" type="button" class="btn btn-primary btn-block" value="Add" onclick="addEmpCommission(<?php echo $invoice["product_id"];?>,'<?php echo $invoice["product_description"];?>')" /></td>
                                    </tr>
                            <?php
                                }
                            ?>
    ...
    <script>
        ...
        var checkvalidqty=function(productId){
            ...
            if(parseFloat(productAmount) > 0 && parseFloat(commissionRate) > 0){
                var currentAmount = $("#commissionAmount_"+productId).val();
                $("#commissionInKg_"+productId).val('');
                $("#commissionInKg_"+productId).hide();
                var commissionAmount = parseFloat(productAmount)*parseFloat(commissionRate)/100;
                //totalCommission += commissionAmount;

                $("#commissionAmount_"+productId).val(commissionAmount);
                //$("#totalCommission").val(totalCommission);
            } else if(parseFloat(productAmount) > 0 && parseFloat(commissionInKg) > 0){
                var currentAmount = $("#commissionAmount_"+productId).val();
                $("#commissionRate_"+productId).val('');
                $("#commissionRate_"+productId).hide();
                var commissionAmount = parseFloat(productQty)*parseFloat(commissionInKg);
                //totalCommission += commissionAmount;
